...
var RBACResourceService = function ($http) {
    
    var baseApi =  api_host+'/api/rbac/resource',
        apis = {
            create  : baseApi + '/save',
    ...
    this.create = function(resource){
        if( resource ){
            return $post(apis.create,resource);
        }
    };

    this.delete = function(resource){
        if(resource && resource.id){
           return $post(apis.delete,resource);
        }
    };

    this.update = function(resource){
        if(resource && resource.id){
            return $post(apis.update,resource);
        }
    };
